...
import { IdDuplicadoError } from '../../../errors/customErrors';
...
describe('FormulariosService', () => {
  ...
  beforeEach(() => {
    repoMock = new FormulariosRepository() as jest.Mocked<FormulariosRepository>;
  ...
  it('deve lançar erro ao tentar criar formulário com nome duplicado', async () => {
    ...
    await expect(
      service.createForm({
        nome: 'Form duplicado',
        campos: [{ id: '1', label: 'Campo 1', tipo: 'text', obrigatorio: true }],
      }),
  ...
  it('deve lançar erro de dependência circular', async () => {
    ...
    await expect(service.updateSchemaVersion('formId', payload)).rejects.toThrow(
