...
describe('PUT /formularios/:id/schema_version', () => {
  ...
  beforeAll(async () => {
    ...
    const res = await request(app)
      ...
      .send({
        schema_version: 1,
        nome: uniqueName,
        campos: [{ id: 'idade', label: 'Idade', tipo: 'number', obrigatorio: true }],
      });
  ...
  it('deve criar uma nova versão do schema', async () => {
    ...
    const res = await request(app)
      .put(`/api/v1/formularios/${formId}/schema_version`)
      .send(payload);
  ...
  it('deve detectar dependência circular entre campos calculados', async () => {
    ...
    expect(res.body.mensagem.toLowerCase()).toContain('dependências circulares');
  });
});
