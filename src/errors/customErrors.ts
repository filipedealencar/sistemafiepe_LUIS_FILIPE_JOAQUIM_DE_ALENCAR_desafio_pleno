...
export class FiltroInvalidoError extends ApiError {
  constructor(campo: string, mensagem: string) {
    super(mensagem, 422, 'filtro_invalido', campo);
  }
}
...
export class FormularioProtegidoError extends ApiError {
  constructor(message = 'Este formulário é protegido e não pode ser removido manualmente.') {
...
export class CampoReservadoError extends ApiError {
  constructor(campo: string) {
    super(
      `O campo '${campo}' é calculado automaticamente e não pode ser enviado manualmente.`,
      422,
      'campo_reservado',
      campo,
    );
