...
export class RespostasService {
  ...
  async criarResposta(formularioId: string, payload: any) {
    ...
    const calculated: CampoFormulario[] = form.campos.filter(
      (c: CampoFormulario) => c.tipo === "calculated"
    );
    const enviadosInvalidos = calculated.filter(
      (c) => c.campoId in data.respostas
    );
    if (enviadosInvalidos.length > 0) {
      throw new PayloadInvalidoError(
        `Campos calculados não devem ser enviados: ${enviadosInvalidos
          .map((c) => c.campoId)
          .join(", ")}`
      );
    }
    ...
    const resposta = await this.repo.createResposta(
      formularioId,
      schemaVersion,
      data.respostas
    );
    ...
    for (const campo of form.campos) {
      if (
        campo.condicional &&
        !avaliarCondicional(campo.condicional, data.respostas)
      ) {
  ...
  async removerResposta(
    ...
    const resposta = await this.repo.softDeleteResposta(
      formularioId,
      respostaId,
      usuario
    );
